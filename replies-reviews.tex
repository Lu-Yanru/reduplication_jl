...
Regarding volitionality, it should be noted that, as reported in various grammars, while it is generally true that verbs describing events not controlled by an agent cannot be reduplicated, some of them, under certain circumstances (e.g. in imperative or conditional sentences, or in causative \obj{rang4/jiao4/shi3} sentences, as in ex. (15e) provided by the author(s)), can reduplicate (it might be worthwhile to delve into why this occurs in such sentence structures). As for the possibility of reduplicating stative verbs, this is limited to specific verbs and circumstances. It has been observed that some stative verbs, e.g. \obj{bing4} ‘be sick’, are basically change of state predicates, which means that they are dynamic (see e.g. \citealt[663]{Tham2013}; see also \citealt{Sybesma1997, Liu2010, PeckEtAl2013, Basciano2019}). The same goes for those verbs expressing psychological states which can have a dynamic interpretation, as e.g. \obj{liao3jie3} `understand' \citep[fn. 3]{BascianoMelloni2017}. 
With regard to achievements, it is worth considering whether, in certain circumstances (the cases in which they are reduplicated), they assume a reading that cancels the \textit{telos}. Anyway, since these cases are not the rule, I do not think they can be taken as an argument against the constraints on the action type of the base verb. Otherwise, if reduplication does not affect lexical aspect, how can the changes in the aspect properties of the verb after reduplication be explained? (e.g. 看（了）三个小时 \obj{kan4} \obj{(le)} \obj{san1} \obj{ge} \obj{xiao3shi2} `read for three hours' vs. *看（了）看三个小时 \obj{kan4} \obj{(le)} \obj{kan4} \obj{san1} \obj{ge} \obj{xiao3shi2} (both reduplication and for X time expressions set a boundary to the event) / 正在看 \obj{zheng4zai4} \obj{kan4} / *正在看看 \obj{zheng4zai4} \obj{kan4} \obj{kan}). 
...
The combination with other sentential components such as adjuncts is also discussed in Section 2.3.4.%is briefly touched upon below,
%but a detailed discussion has to be left for future research.

Second, there are more examples.
Regarding volitionality, although (15e) in the previous version of the paper is a causative sentence, (13a--b) in the previous version of the paper (17a--b in the current version) are not imperative, conditional or causative (more on this below).
...
\objex{Wang2} \objex{Ānyu3} is another actor in the series.\\
\\
\gll \objex{Wang2} \objex{Sheng4di2} \ldots\, \objex{tai4} \objex{xiang4} \objex{Gu4} \objex{Miao3} \objex{le}. \objex{Wang2} \objex{Ānyu3} \objex{ni3} \objex{kuai4dian3} \objex{ye3} \obj{xiang4-yi-xiang4} \objex{ba}!\footnotemark\\
...
\obj{Le} expresses a situation being realized,
...
which are ambiguous between words and phrases: e.g. 担心 \obj{dan1-xin1} `take on-heart, worry' has a
...
word, as the author(s) also show(s) in (27c). \citet[1282]{Her2006}, talking about V-给 \obj{gei3} sequences,
states that ``The simple fact that the V-gei sequence cannot be separated, as in (19c) indicates its
lexical integrity'', i.e. they form a single lexical item (寄给了他 \obj{ji4-gei3-le} \obj{ta1} vs. * 寄了给她 \obj{ji4-le} \obj{gei3} \obj{ta1}).
...
Even if you assume that aspect markers are suffixes (morphological elements), they should not be placed in between the constituents of compounds, because this violates lexical integrity (as shown in ex. 27b by the author(s)): the suffix \obj{le} must be placed after the whole complex verb, which forms a single lexical unit, and not within it. In fact, in (non-separable) verb-object compounds, which are lexical units, 了 \obj{le} is never placed after the verbal constituent (关心了 \obj{guan1-xin1-le} / *关了心 \obj{guan1-le} \obj{xin1} vs. 骑了马 \obj{qi2-le} \obj{ma3}).
It is worth noticing that, different from diminishing verb reduplication, in AABB verbal reduplication (which has a different meaning and function), the perfective aspect marker \obj{le} can never be placed inside the reduplicated verb but can be put at the end of the AABB pattern. 
...
3) Phrasal substitution. The inclusion of this test is questionable since reduplication primarily targets verbs and not phrases. Therefore, syntactic objects should not be included in the reduplication; it is not repetition of phrases. Indeed, if the object is part of a non-separable verb-object compound (which is a lexical unit), it is included in the reduplication (关心关心 \obj{guan1-xin1} \obj{guan1-xin1}/*  关关心 \obj{guan1-guan1} \obj{xin1} vs. 跳跳舞 \obj{tiao4-tiao} \obj{wu3} / * 跳舞跳舞 \obj{tiao4wu3} \obj{tiao4wu3}).

4) Conjunction reduction. This criterion can only be applied to set apart coordinated phrases from coordinate compound words with a head in common (e.g. 火车 \obj{huo3che1} e 汽车 \obj{qi4che1} in (33) share the head  车 \obj{che1} `vehicle'). But the examples provided by the author(s) to argue against conjunction reduction with reduplicated verbs are completely different: the verbs do not have the head in common (exx. 34). I believe that this is not a reasonable test to argue for the wordhood of reduplicated verbs.
...
Regarding the position of \obj{le}, there are counter-examples that show V-\obj{le-gei3} (\ref{ex:Vgei}) as well as \obj{guan1-le-}\obj{xin1} `close-\textsc{pfv}-heart, care for' (\ref{ex:guanxin}) are possible.
...
%%%%%%%%%%%%%%%%%%%%%%
%% YL
\item p. 26, lines 24--25: ``The output reduplicates the phonology (phon) of the input verb with the possibility to have further phonological material in between. \etag indicates an underspecified list which could be empty or not''. However, it is not entirely clear what prevents phonological material other than \obj{yi} and \obj{le} to appear in between the reduplicated elements. This aspect seems to be somewhat arbitrary, and it is not evident why certain elements are permissible while others are not.

%%%%%%%%%%%%%%%%%%%%
\textbf{Answer:}
\obj{le} `\textsc{pfv}' marks the perfective aspect.
The reason for \obj{le} `\textsc{pfv}' being the only aspect marker compatible with reduplication is explained in the Section 2.3.3 concerning the semantics of the reduplication and the aspect markers.
...
\gll \objex{ming2} \objex{you4} \objex{he4}, \objex{qi2} \objex{yi4} \objex{he4}, \objex{ming2} \objex{lian2} \objex{he4} \objex{liang3} \objex{he4}, \objex{qi2} \objex{bian4} \objex{li3bai4}\footnotemark\\
...
\z
\footnotetext{\obj{Song4dai4} \obj{juan4}: \obj{Zhang1} \obj{Xie2} \obj{zhuang4yuan2} \textit{[The Song dynasty volume: Top graduate Zhang Xie]} p. 519 as cited in \citet[13]{Zhang2000}}
...
%%%%
\item
p. 18, exx. (39b): to the best of my knowledge, the object can be placed before 一下 \obj{yixia} only in limited cases (pronouns and proper nouns).
%%%%%%%%%%%%%
...
This is probably the case for those AABBs that have an idiomatic meaning, such as \obj{zhi3-zhi3-dian3-dian3} `point-point-gesture-gesture, gossip'.
...
\ea[*]{\gll ta dakai qingsheng de men.\\
         he open quietly       \textsc{de} door\\
\glt         'He opened the door quietly.'}
\z
...
\begin{displayquote}
R2 comments on the following passage: Following the analyses above, it is clear that the reduplication failed all of the tests for phrasal status. In comparison, Parallel Verb Compounds failed all the tests as well, whereas SVCs passed all of them. This makes the behavior of the reduplication seem more similar to that of a Parallel Verb Compound than that of an SVC. Therefore, it seems more appropriate to assume reduplication to be a morphological process rather than a syntactic one.
\end{displayquote}
...
%%%%%%%%%%%%%%%%%%%
\textbf{Answer:} 
See the answer to Reviewer~\ref{sec:1}, point~\ref{morph} on page~\pageref{morph} for a detailed answer. 
...
\item
\begin{displayquote}
R2 comments on the following passage: Third, idioms (43a) lose their idiomatic meaning when used with verbal classifiers (43b), but maintain their idiomatic meaning with reduplications (43c)
\end{displayquote}
...
\type{perfective-reduplication-lr} in (51), which says that the PHON value of the daughter \ibox{1} appears
...
This term is first used in this sense by \citet{Agrell1908}. %78
