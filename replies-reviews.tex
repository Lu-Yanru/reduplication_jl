...
Due to its unique features compared to the perfective aspect marker \textit{le} and the experiential aspect marker \textit{guo4},
...
\z
\footnotetext{《宋代卷·虚堂和尚语录》3 87 页) as cited in \citet[12]{Zhang2000}}
...
\begin{enumerate}
\item This paper presents an innovative analysis of verb reduplication in Mandarin Chinese, demonstrating
...
\item Examples (27) and (28) serve as illustrations to demonstrate the application of the 'Lexical Integrity Hypothesis' in distinguishing PVC from SVC.
...
\item In Section 2.4, the authors asserted that verb reduplication is more phrase-like than word-like based on its similarities with Parallel Verb Compounds as apposed to Serial Verb Construction (SVC). 
...
But \textit{yi xia} `once' has acquired a duration reading `for a little while' that is not available to the other event quantifiers formed by \textit{xia}, which only have the `for X times' interpretation (\citealt[77]{Deng2013}, \citealt[16]{Zhang2000}).
We think \textit{liang xia} `twice' is following this tendency, too.
In this case, it is easy to interpret \textit{yi xia} and \textit{liang xia} not as referring to the actual number of action taking place, 
...
%% YL
\begin{enumerate}
\item The following paragraph looks terse to me. The author(s) want to provide more explanation about the four tests. In addition, the footnote 6 might be better to move into the body.
