...
%%%%%%%%%%%
\subsection{The reduplicant as a verbal classifier}\label{sec:clf}

\citet{Chao1968}, \citet{Fan1964} and \citet{Xiong2016} analyzed the reduplication in Mandarin Chinese
...
not tell \textsc{exp} one \textsc{clf} lie\\ \jambox{\citep[616]{Chao1968}}
...
Dynamic and volitional verbs have the features [init proc] and are therefore located unter \textit{init} and \textit{proc}  \citetext{\citealp[24]{Arcodiaetal2014}; \citealp[147]{BascianoMelloni2017}}.  
...
Consequently, this proposal does not seem to offer an appropriate account for the reduplication.
 ...
 \citet{YangWei2017} endorsed the analysis of the reduplication as an aspect marker following the structure of Mandarin Chinese aspects proposed by \citet{Tsai2008}.
...
Based on this, the reduplicant should reside under Asp$_2$.
...
\glt `He laughed a little, didn't speak.'}\label{ex:redup-tsai-co}
\z
\z

This analysis would result in a mismatch between syntax and semantics. 
Even though reduplication, \textit{le} and \textit{guo} all mark perfective aspects (\sectref{sec:aspM}, \citealt{Dai1997, XiaoMcEnery2004}),
\textit{guo} is situated under Asp$_1$ while \textit{le} and the reduplication are under Asp$_2$.
...
This analysis correctly captures the fact that the addition of \textit{yi} does not change the syntactic and semantic behaviors of the reduplication.
It also provides a formal account for the phonology of the reduplication.
On the other hand, by assuming a construction specially for the reduplication, this approach fails to account for the similarities of the reduplication and other aspect markers in Mandarin Chinese, unlike the affixation analyses described in \sectref{sec:asp}.
...
Thus, these forms should not be handled as  irregular forms, but should be derivable from lexical rules.
