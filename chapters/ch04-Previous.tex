...
One reviewer suggests that the loss of the idiomatic meaning in (\ref{ex:clf-idiom}) but not in (\ref{ex:redup-idiom}) may be attributed to the use of the numeral \obj{san1} `three' in (\ref{ex:clf-idiom}),
because if \obj{san1} \obj{xia3} `three times' is replaced with \obj{yi2} \obj{xia4} `once' in (\ref{ex:clf-idiom}), he/she can still get the idiomatic interpretation.
...
He observes that  \obj{zhe} `\textsc{prog}', \obj{le} `\textsc{pfv}' and \obj{wan2} `\textsc{compl}' necessarily occur with additional information about the event denoted by the sentence (\ref{ex:incomplete}), 
while \obj{zai4} `\textsc{dur}' and \obj{guo4} `\textsc{exp}' can occur without further information (\ref{ex:complete}).
...
\gll \objex{Xiao3li3} (\objex{gang1cai2}) \objex{zai4} \objex{ku1}/\objex{ku1}-\objex{guo4}.\\
Xiaoli just.now \textsc{dur} cry/cry-\textsc{exp}\\
\glt `Xiaoli was crying/cried just now.'
\z
He thus proposed three aspect positions under TP. \obj{zai4} and \obj{guo4} reside under Asp$_1$, while \obj{zhe} and \obj{le} under Asp$_2$, as illustrated in \figref{tree:tsai}.\footnote{\citet{Tsai2008} differentiated the middle and the inner aspect based on the fact that \obj{wan2} can only occur with certain types of predicate. This differentiation does not play a role for our purpose and will not be further discussed here.}
...
Turning to reduplication, it cannot occur without additional sentential elements as well (\ref{ex:redup-tsai}).
...
Even though \obj{le}, \obj{guo4} and reduplication all mark perfective aspects (\sectref{sec:aspM}, \citealt{Dai1997, XiaoMcEnery2004}),
\obj{guo4} is situated under Asp$_1$ while \obj{le} and the reduplication are under Asp$_2$, as can be seen from \figref{tree:redupasp}.
