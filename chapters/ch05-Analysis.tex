...
using MRS, as proposed in \citet{LuMueller2021}.
...
A delimitative relation is appended to the \textsc{rels} value of the input verb, 
...
hence \type{delimitative-rel} is treated as a modifier.
 Further relations can be added at the beginning of the \textsc{relations (rels)} list to allow for
...
the daughter plus one element. 
Since the element is specified in the supertype, it has not been specified in (\ref{avm:nonperf}) again. 
...
\scalebox{.9}{%
    \avm{
        [\type*{perfective-lr}\\
        phon & \1 \+ \phonliste{ le }\\
        \punk{synsem\textbar cont}{ [ ltop & \2\\
            ind & \3 ]}\\ 
...
\scalebox{.9}{%
    \avm{
        ...
        \punk{synsem\textbar cont\textbar ltop}{ \2}\\ 
        rels & <[\type*{perfective-rel}\\
        lbl  & \2\\
        arg0 & \3\\
...
Indeed, a few examples of AB\hyp{}\emph{yi}\hyp{}AB and AB\hyp{}\emph{le}\hyp{}\emph{yi}\hyp{}AB in Early Mandarin (12th to 20th centuries) (\ref{ex:AByiAB-lu})--(\ref{ex:AByiAB-pu}) and Modern Mandarin (\ref{ex:AByiAB-rou1})--(\ref{ex:ABleyiAB-ccl}) were found.
...
\footnotetext[20]{Li, Jieren. 1962. \textit{Da bo [Great wave]}, 3rd band, 171. Beijing: The Writers Publishing House.}
