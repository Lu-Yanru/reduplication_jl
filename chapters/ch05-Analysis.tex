...
many scattered-\textsc{dur} \textsc{de} boat in \textsc{de} light also flicker \textsc{de} change-\textsc{pfv}-one-chang position\\ \hfill {(CCL)}
...
special power \textsc{de} researcher-\textsc{pl} may.as.well such try-one-try-look\\ \hfill {(CCL)}
...
but about issue not issue-\textsc{exp} conformity-certificate already say not clealy \textsc{ptc} need check-one-chek-look\\ \hfill {(CCL)}
...
let I come guess-one-guess-look\\ \hfill {({CCL})}
\glt `Let me have a guess.'

\ex
\gll da-laoban-men yao \textit{deng-yi-deng-kan}\\
big-boss-\textsc{pl} need wait-one-wait-look\\ \hfill {(CCL)}
...
madam you just count-one-count-look this \textsc{clf} flower \textsc{de} blossom in.total have how.many \textsc{clf} color\\ \hfill {(CCL)}
