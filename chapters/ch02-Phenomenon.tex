...
The reduplication cannot be aspect marked, though, except with the perfective aspect marker \textit{le} (see \sectref{sec:aspM}).% for further discussions on interactions with aspect markers). 
The reduplication also cannot co-occur with an expression that quantifies the duration or the extent of the event described by the sentence due to its semantic properties (see \sectref{sec:adjuncts}).
...
We will then investigate the interaction of the reduplication with verb classes (\sectref{sec:Aktionsarten}), adjuncts (\sectref{sec:adjuncts}) and aspect markers (\sectref{sec:aspM}).
...
\\
\gll \objex{Wang2} \objex{Sheng4di2} \ldots\, \objex{tai4} \objex{xiang4} \objex{Gu4} \objex{Miao3} \objex{le}. \objex{Wang2} Ān\objex{yu3} \objex{ni3} \objex{kuai4dian} \objex{ye3} \obj{xiang4-yi-xiang4} \objex{ba}!\footnotemark\\
